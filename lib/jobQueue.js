  ...
  try {
    ...
    if (cld.isConfigured()) {
      const [logoUrl, ownerUrl] = await Promise.all([
        cld.getAssetUrl('restaurant-social-ai/logo').catch(() => null),
        cld.getAssetUrl('restaurant-social-ai/owner').catch(() => null),
      ]);
      if (logoUrl)  config._logoUrl  = logoUrl;
      if (ownerUrl) config._ownerUrl = ownerUrl;
    }

    // Load script text from linked post's template (if any)
    let scriptText = null;
    if (job.scheduledPostId) {
      ...
      if (post?.scriptTemplate?.scriptText) scriptText = post.scriptTemplate.scriptText;
    }
