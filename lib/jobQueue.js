...
async function processNextJob() {
  ...
  try {
    ...
    const config = {
      ...
      ownerVoiceId:   restaurant.ownerVoiceId || null,
      voiceTone:      restaurant.voiceTone    || null,
    ...
    if (cld.isConfigured()) {
      ...
      const fetches = [
        ...
        // Legacy shared folder only for restaurant 1 — never bleed into other accounts
        (needsPhotos && job.restaurantId === 1) ? cld.listFolder('restaurant-social-ai/photos/').catch(() => []) : Promise.resolve([]),
      ];

      const [logoUrl, ownerUrl, perRestaurantPhotos, sharedPhotos] = await Promise.all(fetches);

      // Logo: per-restaurant path first, then legacy unscoped path only for restaurant 1
      const resolvedLogoUrl = logoUrl
        || restaurant.logoUrl
        || (job.restaurantId === 1 ? await cld.getAssetUrl('restaurant-social-ai/logo').catch(() => null) : null);
      // Owner: DB portrait URL first, then per-restaurant Cloudinary, then legacy only for restaurant 1
      const resolvedOwnerUrl = restaurant.ownerPortraitUrl
        || ownerUrl
        || (job.restaurantId === 1 ? await cld.getAssetUrl('restaurant-social-ai/owner').catch(() => null) : null);

      if (resolvedLogoUrl)  config._logoUrl  = resolvedLogoUrl;
      if (resolvedOwnerUrl) config._ownerUrl = resolvedOwnerUrl;
      if (restaurant.heygenAvatarId) config.heygenAvatarId = restaurant.heygenAvatarId;
