...
// Called by cron every 5 minutes — publishes all due posts for restaurants with auto-publish on.
async function runAutoPublish() {
  try {
    const enabledRestaurants = await db.restaurant.findMany({
      where:  { autoPublishEnabled: true },
      select: { id: true },
    });
    if (!enabledRestaurants.length) return;

    const enabledIds = enabledRestaurants.map(r => r.id);
    const now        = new Date();
